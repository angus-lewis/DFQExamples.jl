...
ks_data = CSV.read((@__DIR__)*"/../data/ks_cdf_func_count_4.csv",DataFrame)
...
l2_pdf_data = CSV.read((@__DIR__)*"/../data/l2_pdf_func_count_4.csv",DataFrame)
...
for (c,o) in enumerate(os)
    ...
    d0 = SFMDistribution((x,i)->1.0,dq,TrapezoidRule;fun_evals=10_001)    
    plot!(xs,pdf(d0,unnormalised_closing_operator_pdf).(xs,1),subplot=c,label=(c==1 ? "unnormalised" : false),
        ...
        linewidth=2)
    plot!(xs,ones(size(xs)),subplot=c,label=(c==2 ? "true PDF" : false),
