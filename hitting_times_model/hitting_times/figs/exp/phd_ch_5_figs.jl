...
# sim_cdf = ht_cdf(sim_data).(data.t[1:T_idx],2)
@show maximum(abs.(data.phase_2-sim_cdf))
@show findall(maximum(abs.(data.phase_2-sim_cdf)).==abs.(data.phase_2-sim_cdf))
linestyles_vec = [:solid,:dash,:dashdot, :dot]
plot(data.t[1:T_idx],data.phase_2[1:T_idx],label="Order 21, DG",color=colours[1],linestyle=linestyles_vec[1],linewidth=2)
data = CSV.read((@__DIR__)*"/../../data/exp/order_21_model_qbdrap4.csv",DataFrame)
@show maximum(abs.(data.phase_2-sim_cdf))
@show findall(maximum(abs.(data.phase_2-sim_cdf)).==abs.(data.phase_2-sim_cdf))
plot!(data.t[1:T_idx],data.phase_2[1:T_idx],label="Order 21, QBDRAP",color=colours[4],linestyle=linestyles_vec[4],linewidth=2)
plot!(data.t[1:T_idx],sim_cdf,label="Sim",color=5,linestyle=:dash,linewidth=2)
plot!(xlabel="t")
plot!(ylabel="Probability")
plot!(title="First exit time approximations, phase 2")
