...
sim_cdf = CSV.read(
    "/Users/anguslewis/Documents/SFFMProject/DFQExamples.jl/hitting_times_model/reflecting_model/transient_distribution/point_mass/datat2/sims_cdf_evaluated.csv",
    DataFrame,
)

for o in orders
        approx_data = CSV.read(
            (@__DIR__)*"/reflecting_model/data/"*ic*"/data2/cdfs/order_"*string(o)*"_model_limiter.csv",
            DataFrame,
        )
        ks_error = max(
...
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/ks_error.csv",ks_errors)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_error.csv",l1_errors)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_cell_probs_error.csv",l1_cell_probs_errors)
...
for o in orders
    # for (c,m) in enumerate(string.(approx_types[[1;3;4]]).*string.([1;3;4]))
        ...
        for n in 1:n_boot
            sim_cdf = CSV.read(
                "/Users/anguslewis/Documents/SFFMProject/DFQExamples.jl/hitting_times_model/reflecting_model/transient_distribution/point_mass/datat2/bootstrap/sample_"*string(n)*".csv",
                DataFrame,
            )
            ks_error = max(
                maximum(abs.(approx_data.phase_2-sim_cdf.phase_2)),
...
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/ks_error_lwr.csv",ks_errors_lwr)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_error_lwr.csv",l1_errors_lwr)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_cell_probs_error_lwr.csv",l1_cell_probs_errors_lwr)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/ks_error_upr.csv",ks_errors_upr)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_error_upr.csv",l1_errors_upr)
CSV.write((@__DIR__)*"/reflecting_model/data/"*ic*"/data2/errors/l1_cell_probs_error_upr.csv",l1_cell_probs_errors_upr)
