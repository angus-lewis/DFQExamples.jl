...
function expected_crossings(model::BoundedFluidQueue)
    rate_reverse_model = BoundedFluidQueue(model.T,-rates(model),model.P_lwr,model.P_upr)
...
function B_matrices(model::BoundedFluidQueue)
    Λ̂₁₁, Ψ̂₁₂, Ψ̂₂₁, Λ̂₂₂ = hat_matrices(model)
    S, ~ = DiscretisedFluidQueues._model_dicts(model)
    n = n_phases(model)
...
function stationary_distribution_x(model::BoundedFluidQueue)
    ...
    ν = DiscretisedFluidQueues.nu(model)
    S, T = DiscretisedFluidQueues._model_dicts(model)
    Cinv = LinearAlgebra.diagm(1.0./[rates(model)[S["+"]];abs.(rates(model)[S["-"]])])
    n = n_phases(model)

    # density matrix 
    N = DiscretisedFluidQueues.expected_crossings(model)

    # cdf matrix
    int_N = DiscretisedFluidQueues.int_expected_crossings(model)

    # boundary probabilities
    p = DiscretisedFluidQueues.point_mass(model)
    n_upr = n-length(S["-"])
    p_upr = p[1:n_upr]'
    p_lwr = p[n_upr+1:end]'
